...
@Component({
  ...
  templateUrl: './mix-data.html',
  styleUrls: ['./mix-data.scss']
})
export class MixDataComponent implements OnInit {
  data: MixData[] = [];
  allData: MixData[] = [];
  loading = false;
  ...
  exportCSV() {
    ...
    this.saveBlob(new Blob([lines.join('\n')], { type: 'text/csv;charset=utf-8;' }), `mix_report_${Date.now()}.csv`);
  }
  ...
  exportXLSX() {
    ...
    this.saveBlob(new Blob([wbout], { type: 'application/octet-stream' }), `mix_report_${Date.now()}.xlsx`);
  }
  ...
  exportPDF() {
    ...
    doc.save(`mix_report_${Date.now()}.pdf`);
  }
